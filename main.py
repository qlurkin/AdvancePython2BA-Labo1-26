...
def create_galaxies():
    return [
        ...
        Galaxy(
            Name="Small Magellanic Cloud", MegaLightYears=0.2, GalaxyType=GType("L")
        ),
...
def IterateThroughListByDistance(theGalaxies):
    sorted_galaxies = sorted(
        theGalaxies, key=cmp_to_key(lambda g1, g2: g1.compare_by_distance(g2))
    )
...
def find_spiral_galaxies(galaxies):
    return [
        g
        for g in galaxies
        if hasattr(g.GalaxyType, "is_spiral") and g.GalaxyType.is_spiral()
    ]
...
def find_galaxy_by_distance(galaxies, target_distance):
    for galaxy in galaxies:
        ...
        if math.isclose(galaxy.MegaLightYears, target_distance):
            return galaxy
    return None
